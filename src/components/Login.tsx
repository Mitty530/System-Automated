...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const demoUsers: User[] = [
    {
      ...
      email: 'salmansouri@quandrox.com',
      role: 'admin',
      department: 'Loan Administrator',
      region: 'UAE',
      permissions: ['create_requests', 'approve_reject', 'disburse', 'view_all']
    },
    {
      id: 'finance_001',
      name: 'Ahmed Hassan',
      email: 'ahassan@quandrox.com',
      role: 'finance_manager',
      department: 'Core Banking',
      region: 'UAE',
      permissions: ['create_requests', 'approve_reject', 'view_department']
    },
    {
      id: 'ops_001',
      name: 'Fatima Al-Zahra',
      email: 'falzahra@quandrox.com',
      role: 'operations_lead',
      department: 'Operations Team',
      region: 'UAE',
      permissions: ['create_requests', 'view_department']
    }
  ];
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Authenticate using demo users only
      console.log('🔄 Authenticating with demo users...');
      ...
      if (!user) {
        setError('Invalid credentials or department mismatch. Please check your email and department selection.');
        return;
      }
        ...
        <div className={`premium-login-panel p-6 rounded-2xl ${mounted ? 'fade-in' : ''}`}>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                required
                className="premium-input"
                style={{ color: department ? '#1f2937' : '#9ca3af' }}
              >
                <option value="" disabled>Select your department</option>
                {departments.map((dept) => (
                  <option key={dept} value={dept} style={{ color: '#1f2937' }}>{dept}</option>
                ))}
              </select>
            </div>
          ...
          <div className="mt-6 text-center">
            <button
              ...
              onClick={() => {
                // For now, show an alert - can be replaced with actual password reset functionality
                alert('Password reset functionality would be implemented here. Please contact your system administrator for password reset assistance.');
              }}
            >
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
